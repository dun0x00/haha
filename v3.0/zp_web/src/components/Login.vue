...
export default {
    ...
    methods: {
        ...
        async submit() {
            ...
            const {data:res} = await login(`account=${this.account}&pwd=${this.pwd}`)
            if (res.code == 200){
                // this.isShowLoading = true
                // 登陆成功 设置用户信息
                localStorage.setItem('userImg', res.session[0].img || 'https://avatars3.githubusercontent.com/u/22117876?s=460&v=4')
